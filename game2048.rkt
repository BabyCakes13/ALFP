...
; if the game is not detected as won yet, if there are no more zero tiles and if there are no
; collosions of tiles with the same value on horizontal / vertical, the game is considered lost.
(define (isLost? game)
  (if (and
       (and (eq? (isWon? game) #f) (eq? (find 0 (flatten (Game-board game))) #f))
       (eq? (can-increase-score (Game-board game)) #t)
       )
      #t
      #f
))
...
(define (moveDown game)
  ...
  (Game (reverse (transpose (map cdr rows-with-score))) 
        (foldr + (Game-score game) (map car rows-with-score))) 
  )
